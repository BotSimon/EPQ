...
def search():
    ...
    if form.validate_on_submit() == False:
        ...
        author_name_list = search_author.split(' ')
        title_word_list = search_title.split(' ')
        reviewer_name_list = search_reviewer.split(' ')

        author_names = "'{}'".format("','".join(author_name_list))
        reviewer_names = "'{}'".format("','".join(author_name_list))
        title_words = "'{}'".format("','".join(author_name_list))
        title_string = "'{}'".format(search_title)
        ...
        SQL = ( "SELECT *"
                ...
                " AND (authors.first_name IN ({author_names}) OR authors.last_name IN ({author_names}))"
                " AND (books.title IN ({title_string}))")

        SQL = SQL.format(author_names=author_names
...
def write():
    ...
    if form.validate_on_submit():
        ...
        author = Person.query.filter_by(first_name=form.author_first_name.data).filter_by(
            last_name=form.author_surname.data).first()
        if author is None:
